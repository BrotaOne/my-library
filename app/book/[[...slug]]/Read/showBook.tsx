...
const ShowBook = async ({params}: {params: Promise<{slug?: string[]}>}) => {
    ...
    const {fileType, fileUrl} = handleSlug(slug)

    if (!fileType) {
        return <div>还未选择文件</div>
    }

    if (fileType === 'pdf') return <PDFReader Header={Header} key={fileUrl} />

    if (fileType === 'epub') return <EpubReader Header={Header} key={fileUrl} />
